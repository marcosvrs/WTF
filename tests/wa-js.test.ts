...
const sendMessage = async (page: Page, payload: Message) => {
  ...
  const parent = page
    .locator('div[data-testid^="list-item-"]')
    .filter({ hasText: process.env.TEST_CONTACT });
...
const assertButtons = async (
  ...
  const messageParent = page
    .locator(
      `div[data-testid^="conv-msg-true_${process.env.TEST_CONTACT?.replace(/\D/g, "")}@"]`,
    )
    .filter({ hasText: message });
  await Promise.all(
    buttons.map(async (button) => {
      const buttonContainer = messageParent
        .getByRole("button")
        .filter({ hasText: button.text });
      await expect(buttonContainer).toBeVisible();
      if (button.url || button.phoneNumber) {
        const span = buttonContainer.getByTestId(
          button.url ? "hsm-link" : "hsm-call",
        );
...
const convertFileToAttachment = async (
  filePath: string,
...
test.describe.serial("Send Messages via WPP", () => {
  test("expect", async ({ page }, testInfo) => {
    ...
    qrCodeContainer
      .waitFor()
      .then(async () => {
        qrcode.generate(await qrCodeContainer.getAttribute("data-ref"), {
          small: true,
        });
      })
      .catch((e) => {
        console.log(
          "QR Code Error: ",
          e.message || e.name || e || "Unknown error",
        );
      });
    ...
    await test.step("message with MP4 attachment to be sent", async () => {
      ...
      const parent = await sendMessage(page, {
        contact: process.env.TEST_CONTACT?.replace(/\D/g, "") || "",
        message,
        attachment: await convertFileToAttachment(
          "wa-js.test.ts-snapshots/sample.mp4",
        ),
        buttons: [],
      });
      await parent.click();
      const container = await assertImage(
        page,
        message,
        "sample.mp4.png",
        "video-content",
